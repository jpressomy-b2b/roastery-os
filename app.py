...
# --- v178.0 FULL STEALTH CSS INJECTION ---
st.markdown("""
    <style>
...
st.title("🔥 Jpresso Roastery Intelligence v178.0")
st.caption("The Master Blueprint: Complete Features & Guaranteed Print Scaling")
...
# ==========================================
# TAB 1: PRODUCTION LEDGER (FULL RESTORATION)
# ==========================================
with tab1:
    ...
    with col_in:
        ...
        wheel_map = {
            ...
            "🧪 Other": [("Other", ""), ("Chemical", "Other"), ("Musty/Earthy", "Other"), ("Woody", "Other")]
        }
        ...
        batch_notes = st.text_area("📝 General Notes")
    ...
    with col_out:
        ...
        if st.session_state.last_analysis:
            ...
            st.markdown(f'<div class="spec-box"><h4>🌟 PRODUCTION SPEC</h4><p><strong>Batch:</strong> {ana["batch_id"]} | <strong>SKU:</strong> {ana["sku"]}</p><p><strong>Sensory:</strong> {", ".join(ana["flavors"])}</p><p><strong>SCA:</strong> {ana["cupping"]:.2f}</p></div>', unsafe_allow_html=True)
            st.dataframe(pd.DataFrame(st.session_state.batch_history), use_container_width=True)

# ==========================================
# TAB 2: BRAND STORYTELLER (FULL DROPDOWNS)
# ==========================================
with tab2:
    st.markdown('<h2 class="section-header">📱 Video Prompt Engine</h2>', unsafe_allow_html=True)
    col_v1, col_v2 = st.columns(2)
    with col_v1:
        scene = st.selectbox("Cinematic Environment", ["The Roastery Floor (Classic)", "Modern Café (Urban)", "High-Speed Racing (Adrenaline)", "Outer Space / Cosmic (Sci-Fi)", "Bustling Local Market (Cultural)", "Urban Megacomplex (Cyberpunk/Corporate)", "Rugged Wilderness (Adventure)"])
        mood = st.selectbox("Visual Mood", ["Premium Moody", "High-Energy Bright", "Warm Cozy", "Neon-Lit Gritty", "Untamed Nature"])
    with col_v2:
        target = st.selectbox("Target Audience", ["B2B Wholesale", "B2C Retail"])
    ...
    if st.button("✨ GENERATE CONTENT", use_container_width=True):
        ...
        visuals = {"The Roastery Floor (Classic)": ["beans pouring into hopper", "flame igniting"], "Rugged Wilderness (Adventure)": ["explorer with mug", "campfire sparks"]}
        v_list = visuals.get(scene, visuals["The Roastery Floor (Classic)"])
...
# ==========================================
# TAB 3: AUTO-EVALUATION PORTAL (FIXED A4)
# ==========================================
with tab3:
    ...
    with col_e1:
        ...
        t_bean = st.text_input("Training Bean", placeholder="Colombia Supremo")
    with col_e2:
        t_prof = st.selectbox("Target Profile", ["Light (Filter)", "Medium (Omni)", "Dark (Espresso)"])
    ...
    if st.button("🏅 Generate Certificate", use_container_width=True) and st_name and t_bean:
        ...
        <div class="cert-container"><div class="border-outer"><div class="border-inner">
            ...
            <div><p style="font-size:14px;">This formally certifies the technical audit of</p><h2 style="font-size:28px; border-bottom:2px solid #d4af37; display:inline-block; padding:0 20px; margin:10px 0;">{st_name.upper()}</h2><p style="font-size:14px;">Completed a formal roast evaluation targeting {t_prof} utilizing {t_bean}.</p></div>
